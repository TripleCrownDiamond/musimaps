...
/** Retire un ©/copyright éventuel en tête de la valeur stockée (évite le doublon). */
function cleanCopyright(value: string): string {
  return value.replace(/^\s*©\s*/, '').trim()
}

/** Tracés SVG (simple-icons, viewBox 0 0 24 24) des réseaux supportés. */
...
export default function Footer() {
  ...
  const footerLogo =
    resolveBrandLogo(
      content.brand.footerLogoLight,
      content.brand.footerLogoDark,
      theme,
    ) ?? (theme === 'dark' ? logoWhite : logoBlack)
  // Hauteur du logo footer (px) contrôlable depuis l'admin (défaut 32).
  const footerLogoHeight = content.brand.footerLogoHeight || 32
  // Liens visibles : la waitlist est masquée pour les utilisateurs connectés.
  const { links, socials, copyright, tagline } = content.footer
  const visibleLinks = loading ? links : user ? links.filter((l) => !l.to.includes('waitlist')) : links
  const copyrightText = cleanCopyright(copyright)
  // Le tagline est déjà contenu dans le copyright stocké ? On ne le répète pas.
  const fullCopyright = copyrightText.includes(tagline)
    ? copyrightText
    : [copyrightText, tagline].filter(Boolean).join(' ')
    ...
    <footer className="border-t border-hairline px-6 py-12 md:px-12">
      <div className="mx-auto flex max-w-7xl flex-col items-center gap-8">
        <div className="flex w-full flex-col items-center justify-between gap-8 md:flex-row md:items-start">
          <Link to={localize('/')} aria-label={t('common.homeAria')} className="shrink-0">
            ...
            <img
              ...
              style={{ height: footerLogoHeight, maxHeight: 140 }}
            />
            <img
              src={theme === 'dark' ? iconWhite : iconBlue}
              alt="Musimaps"
              className="block w-auto shrink-0 md:hidden"
              style={{ height: Math.round(footerLogoHeight * 1.2), maxHeight: 44 }}
            />
          </Link>
          ...
          {socials.length > 0 && (
            <div className="flex items-center gap-3">
              {socials.map((social) => {
                const path = SOCIAL_PATHS[social.icon] ?? SOCIAL_PATHS[social.label.toLowerCase()]
                return (
                  <a
                    key={social.label}
                    href={social.to}
                    ...
                    title={social.label}
                    className="flex size-10 items-center justify-center rounded-full border border-hairline-strong bg-surface text-secondary-text transition-all hover:scale-105 hover:border-brand-deep hover:text-brand-deep"
                  >
                    {path ? (
                      <svg
                        viewBox="0 0 24 24"
                      ...
                      /* Icône générique si le réseau n'est pas reconnu */
        ...
        <p className="border-t border-hairline pt-6 text-center text-sm text-secondary-text">
          © {year} {fullCopyright}
        </p>
      </div>
    </footer>
