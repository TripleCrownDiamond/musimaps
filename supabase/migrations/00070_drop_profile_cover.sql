-- ============================================================
-- 00070 — Suppression définitive des photos de couverture.
--
-- Le produit ne gère plus que la photo de profil (avatar). On retire :
--   - map_artists.cover        (00031, bannière artiste)
--   - profiles.cover_url       (00063, bannière compte)
-- et on recrée les deux RPC du profil revendiqué (00060) sans cover.
-- Les textes légaux (00068) qui mentionnaient la couverture sont
-- corrigés dans la même passe.
-- ============================================================

-- ── get_claimed_profile : sans cover ─────────────────────────────
...
-- ── update_claimed_profile : sans p_cover ────────────────────────
...
-- ── Colonnes ─────────────────────────────────────────────────────
ALTER TABLE public.map_artists DROP COLUMN IF EXISTS cover;
ALTER TABLE public.profiles DROP COLUMN IF EXISTS cover_url;

-- ── Textes légaux (00068) : ôter les mentions de couverture ──────
UPDATE public.site_content
   SET content = replace(
         replace(
           replace(
             replace(content::text,
               ', votre photo de profil et votre image de couverture',
               ' et votre photo de profil'),
             'genres, biographie, photo et couverture',
             'genres, biographie et photo'),
           ', your profile picture and your cover image',
           ' and your profile picture'),
         'genres, biography, picture and cover',
         'genres, biography and picture')::jsonb,
       draft = CASE
         WHEN draft IS NULL OR draft = '{}'::jsonb THEN draft
         ELSE replace(
           replace(
             replace(
               replace(draft::text,
